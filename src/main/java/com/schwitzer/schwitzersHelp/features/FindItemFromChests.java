...
public class FindItemFromChests {
    ...
    public static void init() {
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("Saving chest data before shutdown...");
            saveChestDataToFile();
        }));
    }
    ...
    private static void loadChestDataFromFile() {
        try {
            ...
            if (!Files.exists(CHEST_DATA_FILE)) {
                System.out.println("No chest data file found, starting fresh");
                return;
            }
            ...
            if (loadedChests != null) {
                ...
                System.out.println("Loaded " + allScannedChests.size() + " chests from file");
            }
    ...
    private static void saveChestDataToFile() {
        try {
            ...
            System.out.println("Saved " + allScannedChests.size() + " chests to file");
    ...
    public static void clearAllChests() {
        ...
        System.out.println("Cleared all stored chest data");
    }
    ...
    public static void findItem(String itemName)
    {
        ...
        if (!HypixelUtil.isOnPrivateIsland()) {
            System.out.println("Not on Private Island - item search disabled");
            return;
        }
        ...
        for (ChestContents contents : allScannedChests) {
            ...
            if (matches) {
                if (!contents.chestPositions.isEmpty()) {
                    ...
                    System.out.println("Found item at chest block: " + posToRender + (contents.chestPositions.size() > 1 ? " (double chest, rendering primary only)" : ""));
                    found = true;
                }
            }
        }
        ...
        if (!found)
        {
            System.out.println("Did not find any item with that name");
        }
    }
    ...
    public void onWorldUnload(WorldEvent.Unload event) {
        ...
        blocksToRender.clear();
        System.out.println("World changed - cleared blocksToRender");
    }
    ...
    public void onClientTick(TickEvent.ClientTickEvent event) {
        ...
        if (!hasStoredHash || storedContainerHash != currentHash) {
            System.out.println("Chest contents changed - updating (Hash: " + currentHash + ")");
    ...
    private void scanAndUpdateChestContents() {
        ...
        if (!HypixelUtil.isOnPrivateIsland()) {
            System.out.println("Not on Private Island - skipping chest scan");
            return;
        }

        System.out.println("On Private Island - scanning chest contents");
        ChestContents container = new ChestContents();
        container.fillItemNames();

        boolean dataChanged = false;
        if (allScannedChests.contains(container)) {
            System.out.println("duplicate ChestContainer");
            allScannedChests.remove(container);
            allScannedChests.add(container);
            System.out.println("Overwritten old Chest with new Chest");
            dataChanged = true;
        }
        else
        {
            allScannedChests.add(container);
            System.out.println("Added new Chest to list");
            dataChanged = true;
        }
        ...
        for (BlockPos chestPos : container.chestPositions) {
            if (blocksToRender.contains(chestPos)) {
                blocksToRender.clear();
                System.out.println("Opened targeted chest; cleared blocksToRender");
                break;
            }
        }
    }
    ...
    public static class ChestContents {
        ...
        private void fillItemNames() {
            ...
            System.out.println("Chest blocks at : " + chestPositions);
        }
